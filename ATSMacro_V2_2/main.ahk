...
; ================================================================
;   DenniXD ATS MACRO V2.2 — Combined Double Dungeon + Abandon Village
; ================================================================
; ---------------- INITIALIZE FILES ----------------
InitFiles() {
    ...
    if (!FileExist(seqPath)) {
        FileAppend("; DenniXD ATS Macro V2.2 - Sequences`n; Auto-generated on first run`n", seqPath, "UTF-8")
    }
}
InitFiles()
...
; ================================================================
MyGui := Gui("+AlwaysOnTop -Caption +Border", "DenniXD ATS Macro V2.2")
...
MyGui.AddText("x16 y32 w300", "V2.2  ·  Double Dungeon + Abandon Village")
...
RunDynamicSlots(gmKey) {
    ...
    global CustomSeqs, SlotTriggers, GM_DD, GM_AV, GM_Rift, GM_Raid, GM_Summon, GM_Custom
    global Running, StartX, StartY, EndX, EndY

    gmMap := Map("DD",GM_DD,"AV",GM_AV,"Rift",GM_Rift,"Raid",GM_Raid,"Summon",GM_Summon,"Custom",GM_Custom)
    if (!gmMap.Has(gmKey))
        return
...
Execute_ReturnToLobby() {
    ...
    BlockInput("On")
    MouseMove(1564, 827, 0), Click(1564, 827), Sleep(100)
    Loop 11 {
        MouseMove(1564, 804, 0), Click(1564, 804), Sleep(80)
    }
    Send("{f down}"), Sleep(63), Send("{f up}")
    MouseMove(1296, 572, 0), Click(1296, 572),Click(1296, 572), Click(1296, 572), Sleep(100)
    MouseMove(1718, 726, 0), Click(1718, 726), Click(1718, 726), Click(1718, 726)
    BlockInput("Off")
}
...
Execute_ResetTravelUI() {
    ...
    Click(1296, 572),  Click(1296, 572), Sleep(5000)
...
ReturnToLobby() {
    global CurrentRaidStep, RaidStartTime, PrivateServer
    CurrentRaidStep := 0
    RaidStartTime   := 0
    if WinExist(RobloxTitle) {
        WinActivate(RobloxTitle)
...
CaptureAndSend(IsManualTest := false) {
    ...
    Payload := '{"embeds": [{"title": "DenniXD ATS Macro V2.2","color": 8323327,'
             ...
             . '],"footer": {"text": "DenniXD ATS V2.2  ·  ' . FormatTime(, "HH:mm:ss") . '"}}]}'
    try {
...
GenerateDefaultFiles() {
    ...
    hdr := "; ================================================================`n"
          . "; DenniXD ATS Macro V2.2 — Movement File (auto-generated)`n"
...
RejoinPS() {
    global PrivateServer, RobloxTitle, LastRejoinTime

    OneHour := 3600000  ; 1 hour in ms
    NeedsPSLaunch := (LastRejoinTime == 0 || (A_TickCount - LastRejoinTime >= OneHour))

    if (NeedsPSLaunch) {
        ; 1. Close any running Roblox instances
        ...
        ; 2. Open PS link in browser
        PSLink := PrivateServer
        GuiStatus.Text := "Opening Private Server..."
        Run(PSLink)
        LastRejoinTime := A_TickCount

        ; 3. Wait for Roblox process — relaunch PS link every 20s if not detected
        GuiStatus.Text := "Waiting for Roblox to launch..."
        launchAttempt  := 0
        maxAttempts    := 4   ; 4 x 20s = 80s max before giving up
        detected       := false
        Loop {
            if (!Running)
                return
            ; Poll for 20s in 500ms intervals
            pollDeadline := A_TickCount + 20000
            Loop {
                if (!Running)
                    return
                if (RobloxRunning()) {
            ...
            if (launchAttempt >= maxAttempts) {
                GuiStatus.Text := "Rejoin timed out after " maxAttempts " attempts — continuing anyway..."
                break
            }
            ; Roblox not detected after 20s — relaunch PS link
            GuiStatus.Text := "Roblox not detected — relaunching PS (attempt " launchAttempt + 1 ")..."
            Run(PSLink)
            Sleep(1000)
        }

        ; 4. Close browser tab now that Roblox is running
        if (detected) {
            GuiStatus.Text := "Roblox detected — closing browser tab..."
        }
        ...
        ; 5. Wait for Roblox window title to be ready
        WinWait(RobloxTitle, , 30)
        Sleep(1000)
    } else {
        ; Within 1hr — Roblox already running, no need to relaunch PS
        Remaining := Round((OneHour - (A_TickCount - LastRejoinTime)) / 60000)
        GuiStatus.Text := "Skipping PS rejoin (" . Remaining . "m until next)"
        Sleep(1000)
    }

    ; 5. Only run ResetTravelUI if Roblox is open AND game has loaded in
    if (WinExist(RobloxTitle)) {
        WinActivate(RobloxTitle)
        WinWaitActive(RobloxTitle, , 5)
        ; Wait for game loaded indicator (TextLoaded pattern at ~77,602)
        GuiStatus.Text := "Waiting for game to load..."
        loadDeadline := A_TickCount + 60000  ; wait up to 60s
        gameLoaded   := false
        Loop {
            if (!Running)
                break
            if (A_TickCount > loadDeadline) {
                GuiStatus.Text := "Game load timeout — skipping ResetTravelUI"
                break
            }
            if FindText(&fx, &fy, 27, 577, 127, 627, 0, 0, TextLoaded) {
                gameLoaded := true
                GuiStatus.Text := "Game loaded — waiting 10s before ResetTravelUI..."
                break
            }
            Sleep(500)
        }
    ...
    ; 6. Run summon after every rejoin if enabled
    if (ModeSummoning)
        RunSummon()
}
...
global GM_Raid := [
    Map("key","Raid_NamexPlanet",      "label","Namex Planet",      "trigger","—"),
    Map("key","Raid_ColosseumKingdom",  "label","Colosseum Kingdom", "trigger","—"),
    Map("key","Raid_DemonForest",       "label","Demon Forest",      "trigger","—"),
    Map("key","Raid_DungeonTown",       "label","Dungeon Town",      "trigger","—"),
    Map("key","Raid_ReaperSociety",     "label","Reaper Society",    "trigger","—")
]
global GM_Custom := [
...
GetGMSlots(gm) {
    global GM_DD, GM_AV, GM_Rift, GM_Raid, GM_Custom, GM_Summon
    if (gm == "DD")
        return GM_DD
    if (gm == "AV")
        return GM_AV
...
OpenSequenceEditor() {
    ...
    EditorGui := Gui("+AlwaysOnTop -MaximizeBox", "Gamemode Editor — DenniXD ATS V2.2")
    ...
    global DdlEditorGM := EditorGui.AddDropDownList("x100 y55 w180", ["Double Dungeon","Abandon Village","Rift","Raid","Summon","Custom"])
    DdlEditorGM.Value := 1
...
OnEditorGMChange(ctrl, *) {
    ...
    gms := ["DD","AV","Rift","Raid","Summon","Custom"]
...
RefreshSlotLV() {
    global EditorSlotLV, EditorGamemode, CustomSeqs, SlotTriggers, GM_DD, GM_AV, GM_Rift, GM_Raid, GM_Summon, GM_Custom
...
SelectSlot(idx) {
    ...
    global GM_DD, GM_AV, GM_Rift, GM_Raid, GM_Summon, GM_Custom
    slots := GetGMSlots(EditorGamemode)
...
EditorCaptureMouse() {
    ...
    edWin := WinExist("Gamemode Editor — DenniXD ATS V2.2")
    if (edWin && mWin == edWin)
        return
...
SaveEditorSequence() {
    ...
    } else if (EditorGamemode == "Raid") {
        ; Each raid map has its own file
        fname := StrReplace(EditorSlotKey, "Raid_", "") . ".txt"
...
SaveMovementFileSlots(path, keys) {
    global CustomSeqs, SlotTriggers
    out := "; DenniXD ATS V2.2 — saved from editor`n`n"
    for slotKey in keys {
