...
MainLoop() {
    ...
    global AutoRejoinEnabled, IniFile
    if (!Running)
        return
    if (MacroPaused) {
    ...
    ; ── Hourly rejoin check — only if AutoRejoin is enabled and 1hr has elapsed ──
    if (AutoRejoinEnabled && (LastRejoinTime == 0 || (now - LastRejoinTime >= 3600000))) {
